/**
 * ============================================================
 * WORKFLOW 2 — RÉVISION FORMULAIRE (VERSION CORRIGÉE)
 * ============================================================
 * 
 * NO CRYPTO - Using REST API directly
 */
...
const action = body.action; // 'approve' ou 'revise'
const globalComment = body.globalComment || '';
const productComments = body.productComments || {};

console.log('Workflow 2 Input:', { orderId, action, globalComment });
...
// Créer dans Railway : CLAUDE_API_KEY, BREVO_API_KEY
...
// ===== FONCTION : RÉCUPÉRER DOCUMENT FIRESTORE =====
const getFirestoreDocument = async (docId) => {
  const url = `${FIREBASE_DB_URL}/orders/${docId}`;
  ...
  console.log('Firebase response:', JSON.stringify(response).substring(0, 300));
  
  if (!response.fields) {
    throw new Error('Document not found or no fields');
  }
  ...
  return {
    ...
    revisionsLeft:  parseInt(fields.revisionsLeft?.integerValue || '1'),
    revisionsTotal: parseInt(fields.revisionsTotal?.integerValue || '1'),
    status:         fields.status?.stringValue || 'pending',
    createdAt:      fields.createdAt?.stringValue || '',
    results:        JSON.parse(fields.results?.stringValue || '[]'),
    products:       JSON.parse(fields.products?.stringValue || '[]'),
    config:         JSON.parse(fields.config?.stringValue || '{}'),
    brand:          fields.brand?.stringValue || '',
    globalCategory: fields.globalCategory?.stringValue || '',
    altText:        fields.altText?.booleanValue === true,
...
if (action === 'approve') {
  ...
  await this.helpers.httpRequest({
    ...
    body: {
      ...
      subject: `✅ Fiverr files ready — ${docData.fiverr_order}`,
      htmlContent: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0e0e0e;color:#f0f0f0;padding:32px;border-radius:12px;border:1px solid ${color};">
          <h2 style="color:${color};margin-top:0;">✅ Client approved — files attached 📁</h2>
          <p>Fiverr Order <strong>${docData.fiverr_order}</strong> approved. <strong>${pkgLabel}</strong> CSV files for <strong>${platformList}</strong> are attached.</p>
          <p style="color:#888;font-size:13px;">You can now deliver these files to your client on Fiverr.</p>
        </div>`,
...
Rewrite ALL elements in ${lang}. Respond ONLY with valid JSON:
{
  ...
  await this.helpers.httpRequest({
    ...
    body: {
      sender: { name: "GetSEOBolt", email: "contact@getseobolt.com" },
      to: [{ email: "maximelab888@gmail.com" }],
      subject: `🔄 Revision done — ${docData.fiverr_order} | ${newRevisionsLeft} revisions left`,
      htmlContent: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0e0e0e;color:#f0f0f0;padding:32px;border-radius:12px;border:1px solid ${color};">
          <h2 style="color:${color};margin-top:0;">Revision completed 🔄</h2>
          <p>Fiverr Order <strong>${docData.fiverr_order}</strong>. Client revised. <strong style="color:${color}">${newRevisionsLeft} revision${newRevisionsLeft !== 1 ? 's' : ''}</strong> remaining.</p>
          <div style="background:#1a1a1a;border-left:3px solid ${color};border-radius:8px;padding:20px;margin:24px 0;">
            <p style="color:#888;font-size:12px;margin:0 0 12px;">Send this link to your client on Fiverr:</p>
